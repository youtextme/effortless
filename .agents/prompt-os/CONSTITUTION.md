# Outcome OS / Prompt OS Constitution
...
0b. For checkable-artifact runs, execute the **seven layers** (`pos layers` / `docs/OBJECTIVE-RUNNER.md`): Need → Context → Hypothesis → Truth → Critique → Retrieve → Autonomy.
...
10. **Human Action Closure:** last message before 💪 — ≤80 words, **Done:** + **You:** with actionable link; never stop without the human's next step. Ready PRs only (no draft unless asked). Branch + push + commit before terminal message.

## Compute

Frontier: contracts, architecture, eval judgment, adversarial review.
Local/OpenClaw/Ollama hub: research sweeps, loops, scoring, heartbeats.

## Evolve

After each outcome, append `evolve/log.md`. Promote a pattern into this file only if it survived 3 outcomes and this file stays ≤50 lines. Playbooks become skills, not constitution bloat.
