...
const wordspark = join(root, '..', 'wordspark');
...
// Dependency graph — no cycles, all deps registered
...
for (const contract of ['component.schema.json', 'events.schema.json']) {
...
console.log(`Platform validation OK — ${manifest.components.length} components, kernel ${manifest.kernel.version}`);
